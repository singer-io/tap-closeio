...
class Context():
    ...
    def get_bookmark(self, path):
        return state.get_bookmark(self.state, *path)

    def set_bookmark(self, path, val):
        state.set_bookmark(self.state, path[0], path[1], val)

    def get_offset(self, path):
        off = state.get_offset(self.state, path[0])
        return (off or {}).get(path[1])

    def set_offset(self, path, val):
        state.set_offset(self.state, path[0], path[1], val)

    def clear_offsets(self, tap_stream_id):
        state.clear_offset(self.state, tap_stream_id)
